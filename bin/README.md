...
| `myhost` | Print the locally inferred host class | Unknown hosts fall back to `clio`; do not use it as security or deployment authorization. |
...
| `u [HOST] MAKE_ARGS...` | Dispatch the configured Makefile for Hera, Clio, or Vulcan | On Darwin, first raises launchd and shell file-descriptor limits. It is not a general fleet dispatcher. |
...
| `upgrade-all` | Run broad repository, synchronization, host, and project maintenance | Legacy umbrella operation; suppresses a Hera failure and inherits `update-remote` limitations. It is not a reliable all-host transaction. |
