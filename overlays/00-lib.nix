...
final: prev: {
  ...
  myLib = {
    ...
    mkScriptPackage = { name, src, extraInstallPhase ? ""
      , description ? "Script collection"
      , homepage ? "https://github.com/jwiegley"
      , license ? prev.lib.licenses.mit, platforms ? prev.lib.platforms.darwin
      }:
    ...
    mkSimpleGitHubPackage = { name, version, owner, repo, rev, sha256
      , executable ? name, installDir ? "bin", description
      , homepage ? "https://github.com/${owner}/${repo}"
      , license ? prev.lib.licenses.mit }:
      prev.stdenv.mkDerivation {
        name = "${name}-${version}";
        src = prev.fetchFromGitHub { inherit owner repo rev sha256; };
    ...
    filterGitSource = src:
      ...
      (path: type: type != "directory" || baseNameOf path != ".git") src;
  };
