...
;;
;; This Nix-owned file is deliberately a superset of upstream's
;; tests/anvil-hang-regression-test.el.  Keep the upstream body synchronized
;; while retaining the three deployment-specific shell regressions below.
...
(defun anvil-hang-regression-test--host-clean-p ()
  "Return non-nil when no host transaction or cleanup resource remains."
  (let ((resources (anvil-host--resource-state)))
    (and (zerop (anvil-host--retired-count))
         (null (aref resources 1))
         (null (aref resources 2))
         (null (aref resources 3))
         (not (anvil-host--cleanup-active-p))
         (not anvil-host--cleanup-active)
         (not (timerp anvil-host--cleanup-timer)))))

(defun anvil-hang-regression-test--assert-host-clean ()
  "Wait boundedly for host cleanup, then reject every hidden state leak."
  (let ((deadline (+ (float-time) 1.0)))
    (while (and (not (anvil-hang-regression-test--host-clean-p))
                (< (float-time) deadline))
      (unless (anvil-host--cleanup-active-p)
        (anvil-host--retry-retired-processes))
      (accept-process-output nil 0.01)))
  (unless (anvil-hang-regression-test--host-clean-p)
    (let ((resources (anvil-host--resource-state)))
      (error
       (concat
        "hang regression host cleanup did not converge: "
        "retired=%S staging=%S timer=%S submission=%S "
        "cleanup-state=%S cleanup-mirror=%S timer-mirror=%S")
       (anvil-host--retired-count)
       (aref resources 1)
       (aref resources 2)
       (aref resources 3)
       (car anvil-host--cleanup-state)
       anvil-host--cleanup-active
       anvil-host--cleanup-timer))))

(defun anvil-hang-regression-test--reset-host ()
  "Converge host cleanup without force-clearing ownership state."
  (unless (anvil-host--cleanup-active-p)
    (anvil-host--retry-retired-processes))
...
(ert-deftest anvil-hang-regression-host-quit-cleans-exact-processes ()
  ...
  (let ((real-make-process anvil-host--make-process-primitive)
        ...
        timer shell-process stderr-process quit-seen)
    (unwind-protect
        (cl-letf
              ...
              (lambda (&rest args)
                (let ((process (apply real-make-pipe-process args)))
                  (when (string-prefix-p
                         "anvil-host-stderr-" (plist-get args :name))
                    (setq stderr-process process))
                  process)))
              ...
              (lambda (&rest args)
                (let ((process (apply real-make-process args)))
                  (when (string-prefix-p
                    ...
                    (setq shell-process process))
                  process))))
          (setq timer
                (run-at-time 0.25 nil #'keyboard-quit))
          (condition-case nil
              (anvil-shell "sleep 30" '(:timeout 5))
            (quit (setq quit-seen t)))
          (should quit-seen)
          (should (processp shell-process))
          (should (processp stderr-process))
          (should-not (process-live-p shell-process))
          (should-not (process-live-p stderr-process))
          (should-not (process-buffer shell-process))
          (should-not (process-buffer stderr-process))
          (should (zerop
                   (anvil-host--retired-count)))
      ...
      (when (and (processp shell-process)
                 (process-live-p shell-process))
        (delete-process shell-process))
  ...
  (let ((anvil-host--cleanup-state
    ...
    (unwind-protect
        (let* ((bytes (* 1024 1024))
               ...
               (started (float-time))
               (result
                (anvil-shell command '(:timeout 3 :max-output nil)))
               (elapsed (- (float-time) started)))
