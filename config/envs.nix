...
let myEmacsPackages = import ./emacs.nix pkgs; in rec {
  ...
  projects-env = pkgs.stdenv.mkDerivation rec {
    ...
    srcs = [
      ...
      (import ~/dfinity/formal-models/vote-forecast { inherit pkgs; returnShellEnv = true; })
      (import ~/dfinity/ic-hs {}).ic-hs-shell
      # (import ~/dfinity/ic-monad { inherit pkgs; returnShellEnv = true; })
