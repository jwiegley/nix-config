        ...
        openssh.authorizedKeys = {
          keys =
            ...
            [
              ...
              # ProxyJump opens a direct-tcpip channel; it never execs anything here, so a
              # forced command would not fire. `restrict` disables port forwarding, which IS
              # the capability the jump needs, so it is selectively re-enabled and then pinned
              # to a single destination by permitopen. No shell, no pty, no agent/X11.
              ''from="192.168.1.2",restrict,port-forwarding,permitopen="andoria-08:22" ssh-ed25519 AAAA... pushme-jump@vulcan''
