...
- #124: signed `1ad23d7c` removes both live catalog entries, renderer
  ...
  Key-name-only probes found mutable Claude/Codex configs clear of Ref on Hera,
  VPS, Vulcan, and all four shared-work hosts, but Perplexity is still present:
  `.claude.json` holds `perplexity` on Vulcan, and `context7`, `perplexity`,
  `sequential-thinking` on VPS. Their current managed leaves and PromptDeploy
  manifests still reflect the pre-retirement generation, and Hera's Pi cache is
  stale. Clio timed out. Reconciliation must not run until
