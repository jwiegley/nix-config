...
| Local discovery | Each local provider queries its loopback `/models` endpoint during registration and whenever `/model` refreshes the catalogs, under a 2.5-second per-request bound; a failed refresh retains the last good list. Explicit type, modality, and capability metadata determines model behavior, while missing or unknown metadata falls back to text-only, non-reasoning chat with 262,144 context and 65,536 output |
...
**Basic usage.** On Hera, keep the Nix-managed `org.nixos.omlx` launch agent available and open `/model` to refresh and select the managed `omlx/Qwen3.6-27B-oQ6e-mtp` model, or select `router/sol`. Both expose only `off` and `high`; Shift-Tab cycles between them. Each starts at `off` when no explicit, restored, or global user preference exists; an existing user-selected level still takes precedence. With no enabled thinking level, the transport sends `enable_thinking: false`. On Clio, select only a model returned by `/model` discovery; no fixed Qwen or `router/sol` entry is generated. Linux requires an independently provisioned trusted service and explicit extension loading.
...
- On Hera and Clio, reopen `/model` after the llama-swap or oMLX roster changes; the picker continues to display its cached list while allowing up to 15 seconds for the combined catalog refresh. Each local discovery request retains its separate 2.5-second bound.
