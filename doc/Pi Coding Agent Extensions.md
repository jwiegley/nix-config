...
| Hermes route | `hermes/hermes-agent` at `https://hermes.vulcan.lan/v1`; opt-in, with its bearer credential resolved from password-store only when a request uses the provider and Pi session-affinity headers enabled for stable upstream routing |
