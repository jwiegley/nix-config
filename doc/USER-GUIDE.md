...
its native model catalog; Pi displays its cached model snapshot immediately,
allows `/model` refreshes up to 15 seconds, and gives each local discovery request
a separate 2.5-second bound; mutable auth, history, sessions, caches,
reports, and preferences remain outside generated Nix leaves. The purpose is
one policy authority, not one invented universal client format.
...
no-op or new branch, and every commit newly reachable from the target branch.
...
links with commit-graph and bitmap acceleration disabled, so replacement refs,
grafts, shallow metadata, and alternate-store graph metadata cannot hide a
commit from the range whose exact objects are verified. Transport subprocesses
cannot read mutable system, global, caller-injected, or checkout-local Git
configuration or Git SSH-command overrides; SSH agent authentication remains
environment-owned.
