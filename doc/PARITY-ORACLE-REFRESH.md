...
`test/baseline/parity-<rev>.json`, derived by `test/bin/parity-baseline`. Nearly every
...
this document; the machine-facing half is `test/bin/oracle-currency-test.py`, run on
every commit by `test/bin/quality`'s `python-test` suite.
...
- **Determinism and multiset/drvPath equivalence** — `test/bin/parity-baseline --check`
  / `--compare`. Expensive. Run by a person, or by a refactor's own parity gate.
- **Currency, consistency, provenance, command-currency** —
  `test/bin/oracle-currency-test.py`. Cheap. Run on every commit.
...
`test/bin/parity-baseline --compare` against the current oracle showed one of:
...
test/bin/parity-baseline --compare test/baseline/parity-<old>.json
...
REFRESH_REASON="overlay-factory refactor landed parity-clean (#NN)" \
  test/bin/parity-baseline --refresh test/baseline/parity-<old>.json
...
anything, which is how `test/bin/oracle-currency-test.py` asserts that the oracle's
...
derived quantities, and `test/bin/oracle-currency-test.py` validates them instead.
...
> `test/bin/parity-baseline`. See jwiegley/nix-config#31, reopened for exactly this.


## Enforcement — what happens when the policy is violated

`test/bin/oracle-currency-test.py` fails the build — `test/bin/quality python-test`, hence
...
| The recorded derivation **command diverges** from `test/bin/parity-baseline` | FAIL — the oracle would be derived by a different command than a later gate uses (for example, after the `#47` `config/ai` → `config/fleet` rename). |
...
- **Command currency** SKIPS (loudly, naming this document) until
  `test/bin/parity-baseline --commands` exists; once it does, command drift is a hard
  FAIL.
...
`test/bin/oracle-currency-test.py` ships its own `OracleGuardSelfTests`, which mutate
