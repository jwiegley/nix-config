...
> **Status (2026-07-27):** Historical diagnosis restored after deletion review. The principal same-segment route override is implemented in `config/ssh.nix`; remaining timeout, roaming, regression-test, and Vulcan-hardening decisions are assigned to WU5 before this report may be retired.
...
- **Split-brain DNS.** Forward `vulcan` / `vulcan.lan` → `192.168.3.16` (WiFi), but the SSH alias + reverse DNS of `.1.2` + `gitea` + `router` → `192.168.1.2` (wired). Interactive SSH and build/service traffic (nix remote-builder, retired-proxy, HA) take **different physical paths** to the same box.
