...
32. Preserve the pinned `git-ai` source and overlay for possible re-enablement while identifying and safely retiring only Hera's disabled residual `git-ai bg run` lifecycle; prove it does not return under the tested login/restart condition.
33. Freeze and implement the requested work/personal model matrix, Factory and missing Gemini routes, and DeepSeek v4 Flash through OpenRouter and Factory. Automatic local/premium routing and Claude Max through VibeProxy require approved designs and policy/authentication boundaries before implementation.
