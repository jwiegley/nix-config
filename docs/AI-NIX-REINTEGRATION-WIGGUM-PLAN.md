...
25. Add a version- and digest-guarded one-argument `ExtensionAPI.registerToolRenderer(wrapper)` ABI to the exact Pi 0.81.1 Nix package, matching `pi-quiet`'s wrapper contract. It must compose after normal renderer inheritance for built-in, extension, SDK, and MCP tools; preserve execution and result bytes; cover reload and HTML export; and fall back safely when a wrapper throws.
26. Enforce package conflict policy in executable tests: Ponytail has one canonical six-skill source while its extension is enabled; Dynamic Workflows cannot recursively invoke the existing `subagent`; `pi-lean-ctx` reuses native `lean-ctx` and has one MCP owner; Browser Native uses exactly `agent-browser` 0.33.0 plus declarative Chromium; Hashline and Lens both load while their known immediate-`replace` Lens gap is explicitly reported.
27. Build and load the patched Pi and every requested package on `aarch64-darwin`, `aarch64-linux`, and `x86_64-linux`; run aggregate offline package discovery with failing installer/network sentinels, Home Manager evaluation, authorized activation, fresh Pi introspection, and unchanged mutable credential/session/trust state.
