...
8. Enforce per-machine MCP parity: every MCP available to Claude Code and/or Codex on a machine is also available to OpenCode, Droid, and Pi on that machine. Preserve target-only MCPs. Fresh acceptance must perform a live call through every profile-specific MCP with redacted credential-presence evidence, rather than treating configuration or discovery alone as proof.
...
18. Preserve evidence that the currently installed `pi-subagent` launches headless Pi RPC child processes rather than tmux panes. Correct the earlier local-only conclusion about `pi-btw`: upstream `pi-btw@0.4.1` exists but was omitted from this configuration, and must be audited and packaged declaratively before activation.
...
20. Preserve Codex's Nix-managed LiteLLM default at GPT-5.6 Sol with `ultra` reasoning while retaining writable host-local runtime selection; automated tests and a fresh interactive acceptance session must prove model/reasoning changes no longer collide with immutable Nix configuration. A real hook-trust action must remain trusted across a Codex restart without mutating Nix-owned state.
...
28. Build and load the patched Pi and every requested package on `aarch64-darwin`, `aarch64-linux`, and `x86_64-linux`; run aggregate offline package discovery with failing installer/network sentinels, Home Manager evaluation, authorized activation, fresh Pi introspection, and unchanged mutable credential/session/trust state. The first authorized activation must be followed by an unchanged second activation, and the final report must record exact rollback particulars.
29. Fix the Claude MCP wrapper so variadic commands cannot be parsed as additional `--mcp-config` filenames, and test the real `claude mcp list` parsing boundary while documenting that session-only supplements are not persistent registry entries.
30. Generate Pi `keybindings.json` declaratively from the present mutable file, with exact collision/preflight tests and no ownership of unrelated mutable Pi state.
31. Audit and package immutable `pi-btw@0.4.1`, `@jakeryderv/pi-artifacts`, and `@ygncode/pi-insights` roots. Replace `pi-subagent` completely with pinned `pi-subagentura`; never co-install them, and verify inherited, isolated, asynchronous, cancellation, artifact, and tmux-interactive behavior plus cleanup.
32. Keep Git-AI declaratively configured and pinned but disabled on Hera for possible re-enablement, while identifying and safely retiring only the disabled residual `git-ai bg run` launcher/process lifecycle; prove it does not return under the tested login/restart condition.
33. Freeze and implement the requested work/personal model matrix with exact provider-facing IDs, context limits, output limits, reasoning defaults, and aliases, including Factory and missing Gemini routes and DeepSeek v4 Flash through OpenRouter and Factory. Fresh selector acceptance must prove the intended provider/model/reasoning combinations. Automatic local/premium routing and Claude Max through VibeProxy require approved designs and policy/authentication boundaries before implementation.
34. Decide and enforce whether stock-trader, DEVONthink, and OpenCode may use the attributed Claude routes, then verify post-change usage without exposing request content or credentials.
35. Diagnose the delayed July 23 recordings without reading `~/Library/CloudStorage`, restore transcription and Org import for all four recordings, and verify the resulting notes end to end.
...
13. Run full cross-system package/load checks and authorized activation, then the fresh-tmux cross-host/tool introspection matrix and comprehensive `~/dl` audit, including Codex runtime switching, LiteLLM attribution, Anvil cleanup, and corrected `pi-btw`/subagent conclusions.
14. Close the immediate Claude MCP parsing, Codex hook-trust persistence, Pi keybinding ownership, and residual Hera Git-AI lifecycle defects.
15. Add Pi BTW, Artifacts, and Insights, then replace the old headless-only Pi Subagent with tmux-capable Pi Subagentura under exact package and cleanup contracts.
16. Resolve model-route policy and design gates, implement the approved model matrix and routing, and run fresh selector acceptance without mutating Nix-owned state.
17. Restore the four delayed voice-note transcription/import flows without inspecting CloudStorage, then include their evidence in the final operational report.
