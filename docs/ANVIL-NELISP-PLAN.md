...
- `johnw.anvil.usePerAgentDaemon` defaults to `true` in dedicated mode: each owning Codex OS process gets a supervised root daemon, while that process's two MCP bridges and internal subagents share it. Setting it to `false` selects the single host daemon fallback.
...
- On Darwin with `useDedicatedDarwinEmacs = true`, reuse the pinned MacPort Emacs while loading the pinned Anvil and anvil-ide closures in `anvil-headless-emacs`; default to one supervised daemon per owning Codex process; when `usePerAgentDaemon = false`, install a GUI-domain launchd agent and route its startup diagnostics through the bounded macOS system logger; isolate every daemon's socket, schema cache, root state, and worker state from the interactive session and from other owning processes; and expose the unified 88-tool main registry.
