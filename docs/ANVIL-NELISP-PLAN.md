...
Status: frozen on 2026-07-11, amended on 2026-07-13 after repeated production hangs. This document fixes the objective and the evidence required for completion. It may be amended to record a stricter requirement or a newly discovered fact; it is not to be weakened to accommodate an implementation.
...
- Timeout ordering is a declared cross-client contract: the 45-second heartbeat is shorter than the preserved 120-second cooperative synchronous evaluation budget, which is shorter than the 135-second hard root dispatch watchdog and the 150-second bridge dispatch cap. Synchronous shell tools are capped at 120 seconds and reject larger overrides before process spawn; longer work uses native execution or external asynchronous evaluation. Each external timeout escalates from TERM to KILL after one second, and JSON-RPC metadata parsing has its own two-second cap. Ordinary calls fit parsing, readiness, and dispatch beneath the 210-second tool deadline. Startup uses a separate 20-second initialize-dispatch cap, so the serial 120-second supervisor wait, parsing, 20-second readiness, kill escalations, and initialize remain below the 180-second startup deadline. Runtime overrides may shorten but never enlarge these package maxima. External asynchronous evaluation keeps its independent 300-second default. Promptdeploy render tests bind Codex and Claude to the same 180/210-second client values. Home Manager exports `MCP_TIMEOUT=180000` as a startup fallback, but new processes must inherit it and GUI-launched Darwin clients still require the rendered per-server timeout.
...
16. A repeated stress run performs at least 25 recovery cycles plus concurrent ordinary file, Org, Git, and Elisp calls. It leaves no stale leases, sockets, supervisors, daemons, workers, or duplicated mutations and shows no unbounded latency growth. Current Hera Codex and Claude clients then pass a concurrent soak after complete OS-process restarts; a logical session resume alone is not accepted as activation evidence.
