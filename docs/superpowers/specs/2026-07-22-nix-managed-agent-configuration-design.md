...
- `~/.pi/agent/models.json` contains providers `positron-anthropic`, `positron-google`, `positron-openai`, `nvidia`, `litellm`, `omlx`, and `llama-cpp-local`, with model-level Hera selectors applied. Clio-only `llama-cpp-remote` is excluded.
