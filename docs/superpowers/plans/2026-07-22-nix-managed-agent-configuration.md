...
- Fixed wrapper artifact sets are Claude `nix-managed-settings.json` + `nix-managed-mcp.json`, Codex's single `nix-managed.config.toml` with inline hooks, and Droid `nix-managed-settings.json` + `mcp.json`. A wholly absent set passes through, a wholly usable regular-file set enables managed selection, and any other type/presence state fails closed; `AI_NIX_BYPASS_MANAGED_CONFIG=1` skips only the new injection.
...
- [ ] **Step 1: Build fake upstream packages and write the wrapper matrix first.**
  ...
  The shell test must record argv and environment for Claude and Droid with both artifacts wholly absent, two usable regular-file targets, each one-file mixed state, dangling links, directories, FIFO/non-regular types, and one usable plus one dangling/type-invalid companion. For Codex's single profile artifact, cover absent, usable regular file, dangling link, directory, and FIFO/non-regular states. Across all three wrappers cover conflicting separated, `--flag=value`, `-pNAME`, and `-p=NAME` forms as applicable; explicit bypass in every state; paths containing spaces; upstream nonzero exit propagation; and Codex host-local state in managed and bypass modes. It must also prove error output is bounded/redacted to client name and artifact paths, never file content. For Codex, assert version `0.144.6`; cover empty/default interactive use, ordinary and `import` prompts, every surface that effectively applies profile-v2 (`exec`/`e`, `review`, `resume`, `archive`, `delete`, `unarchive`, `fork`, `sandbox`, and exactly `debug prompt-input`), delegated `mcp` commands, every unsupported top-level command including aliases `a` and `cloud-tasks` plus hidden `execpolicy`, `responses-api-proxy`, and `stdio-to-uds`, explicit root `-h`/`--help`/`-V`/`--version` delegation, separated/attached root option values, variadic separated and attached image forms, `--`, child-command profile-looking arguments, and malformed/unknown option pass-through. Add a real-package smoke using the exact pinned Codex binary: under a synthetic `CODEX_HOME`, prove `debug prompt-input` includes a recognizable `developer_instructions` sentinel only with `--profile nix-managed`; then replace that file with an unknown marker and prove `--profile nix-managed --strict-config exec` fails during local config loading and names that marker before any network action. In the same RED check, add the complete static-header bridge cases specified in Step 6 while `pkgs.agent-http-header-bridge` is still absent.
...
- [ ] **Step 3: Implement one small managed-artifact classifier and the Claude wrapper.**

  The classifier never opens an artifact. For a nonempty path list, it returns `zero` only when every path satisfies `! -e && ! -L`; it returns `complete` only when every path resolves as a usable regular file (`-f`); every dangling link, directory, special type, or mixed state returns `partial` and fails. Check `AI_NIX_BYPASS_MANAGED_CONFIG=1` before classification so explicit recovery always delegates while preserving pre-existing wrapper behavior. Claude uses `${CLAUDE_CONFIG_DIR:-$HOME/.claude}`, injects `--settings <root>/nix-managed-settings.json --mcp-config <root>/nix-managed-mcp.json`, never injects `--strict-mcp-config`, and rejects caller-supplied `--settings` or `--mcp-config` only in complete mode. Add `claude-real` as a direct exec of the original binary.

- [ ] **Step 4: Add managed selection to the existing Codex wrapper without restructuring it.**

  Preserve every current host-local SQLite/log statement. Immediately before its existing final `exec`, classify the single `$CODEX_HOME/nix-managed.config.toml` artifact; a dangling or non-regular path exits with a repair message. In complete mode, use a fixed recognizer for pinned Codex `0.144.6`, not a general parser: inject `--profile nix-managed` only for default interactive use and the surfaces that effectively apply profile-v2: `exec`/`e`, `review`, `resume`, `archive`, `delete`, `unarchive`, `fork`, `sandbox`, and `debug prompt-input`. Delegate `mcp` unchanged because pinned Codex discards the selected layer after migration validation and then operates on mutable base configuration; delegate root `-h`/`--help`/`-V`/`--version`, all other commands, and malformed/unknown option forms likewise. Preserve argv exactly; honor `--`; mirror root option value consumption, including variadic separated `-i`/`--image`; and reject `--profile NAME`, `--profile=NAME`, `-p NAME`, `-pNAME`, or `-p=NAME` only where Codex itself parses that spelling as a config profile. In particular, preserve child-command arguments such as `app-server generate-ts -p`, `sandbox -- echo -p child`, and MCP server command arguments. Assert version `0.144.6` so an upgrade forces table review. Bypass skips this classification/injection but still executes all host-local-state logic.
...
- [ ] **Step 3: Write Phase A and Phase B as disposable operations.**
  ...
  Phase A reuses Task 4's immutable snapshot/oracles, collects protected legacy manifest hashes, compares unchanged assets by framed hash and relocated configs semantically, validates Pi directly, and scans synthetic references/leaks. Phase B classifies each destination. Before mutation, atomically create the backup root on a verified host-local filesystem, reject symlinked parents, verify current-user ownership and mode `0700`, finish every backup, and fsync/complete the journal. The complete pre-mutation journal contains only paths, types, modes, hashes, backup references, and expected pre/post states; only then may exact legacy leaves/owned keys be removed. Mixed files use in-memory redacted structure/key ownership. For Codex, back up the global `$CODEX_HOME/hooks.json`, remove only the exact promptdeploy-owned legacy declarations proven by the ownership mapping, preserve unrelated entries, and stop on any drift or ambiguous ownership; the selected declarations exist only inline in `nix-managed.config.toml`. Any incomplete backup/journal, drift, or unmanaged entry stops; no force path exists.
...
- [ ] **Step 5: Prove commands non-mutatingly and review every checklist row.**
  ...
  Run the recorded evaluation/build commands, Task 4 oracle verification, migration classification against synthetic temporary homes, backup/journal dry runs, Codex global-hook cleanup/restore dry runs with unrelated-entry preservation, a delegated config-loading smoke proving the managed hook sentinel absent, and rollback dry runs without activating or changing live client state. Two independent reviewers map every approved-design requirement to a checklist row and verify every command against its live repository. Keep rollout-only boxes unchecked for Task 13; do not add a permanent checker or flake registration.
...
- [ ] **Step 8: Close the evidence-based observation and retire promptdeploy.**
  ...
  Require every home to pass one normal client-launch cycle, the unchanged second switch, exact managed bytes/symlink targets/store references, and rollback/restore verification. Record explicit rollback-window closure with nonsecret evidence. Remove legacy promptdeploy manifests/marker blocks/bundle receipts and the proven promptdeploy-owned declarations in Codex's global `hooks.json` only from the five migrated clients after protected backups are accepted; preserve unrelated global hooks, GPTel/git-ai artifacts/manifests, and mutable client state. Remove promptdeploy from active instructions/jobs/aliases for these clients, but leave the repository and frozen oracle unchanged as an archive. Remove old GC roots/backups only after the recorded closure decision; never automatically delete later user changes.
