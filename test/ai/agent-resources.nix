...
let
  ...
  piMcpFiles =
    let
      inherit (builtins.fromJSON (builtins.readFile "${piMcpAdapter}/package.json")) files;
    in
    assert builtins.all (
      file: builtins.isString file && builtins.match "[A-Za-z0-9][A-Za-z0-9._-]*" file != null
    ) files;
    lib.subtractLists [ "dist" "skills" ] files;
